...
lazy_static! {
    ...
    static ref STRING_NUM_RE: Regex = Regex::new(r"^(?<string>\D+)\s*(?<num>\d+)$").expect("valid regex");
}
...
fn parse_usage_location(
    ...
    for (key, values) in items {
        ...
        match (key.as_str(), first.take(), second.take()) {
            ...
            ("Rechtszweck:", v, _) => usage_location.legal_scope = v.map(|v| OptionalPair::Single(v)),
            ("East und North:", Some(v), _) => usage_location.utm_northing = Some(v.parse()?),
            ("Top. Karte 1:25.000:", Some(num), Some(s)) => usage_location.top_map_1_25000 = Some((num.parse()?, s)),
            ("(ETRS89/UTM 32N)", Some(v), _) => usage_location.utm_easting = Some(v.parse()?),
            ("Gemeindegebiet:", Some(num), Some(s)) => usage_location.municipal_area = Some((num.parse()?, s)),
            ("Gemarkung, Flur:", None, None) => (),
            ("Gemarkung, Flur:", Some(v), _) => {
                let captured = STRING_NUM_RE.captures(&v).ok_or(anyhow::Error::msg("'Gemarkung, Flur' has invalid format"))?;
                usage_location.local_sub_district = Some(captured["string"].to_string());
                usage_location.field = Some(captured["num"].parse()?);
            }
            ("Unterhaltungsverband:", Some(num), Some(s)) => usage_location.maintenance_association = Some((num.parse()?, s)),
            ("Flurstück:", None, None) => (),
            ("Flurstück:", Some(v), _) => usage_location.plot = Some(v.parse()?),
            ("EU-Bearbeitungsgebiet:", Some(num), Some(s)) => usage_location.eu_survey_area = Some((num.parse()?, s)),
            ("Gewässer:", v, _) => usage_location.water_body = v,
            ("Einzugsgebietskennzahl:", Some(num), Some(s)) => usage_location.basin_no = Some((num.parse()?, s)),
