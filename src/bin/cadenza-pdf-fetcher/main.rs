...
async fn main() {
...

    {
        ...
        while let Err(_) = client.get(browse::CADENZA_URL).send().await {
    ...
    let mut fetched_reports = {
        ...
        BTreeSet::from_iter(find_fetched_reports().expect("could not find already fetched reports").iter().copied())
    };

    let mut unfetched_reports = Vec::new();

    let progress = ProgressBar::new(cadenza_table.rows().len() as u64).with_style(ProgressStyle::with_template("{msg:.cyan}  {wide_bar:.magenta/.234}  {human_pos:.magenta}{slash:.magenta}{human_len:.magenta} {prefix:.cyan} (eta {eta})").expect("is valid schema").with_key("slash", |_: &ProgressState, w: &mut dyn Write| write!(w, "/").expect("write should work here")).progress_chars("━ ━")).with_message("Fetching Reports");
    'wr_loop: for water_right_no in cadenza_table.rows().iter().map(|row| row.no) {
        if fetched_reports.contains(&water_right_no) {
            progress.println(format!("{} {}, already fetched", console::pad_str(console::style("Skipped").green().to_string().as_str(), PRINT_PADDING, Alignment::Right, None), water_right_no));
            progress.inc(1);
            continue;
        }
        ...
        for retry in 1..=(CONFIG.cadenza.retries as u32) {
            ...
            match fetched {
                Ok(_) => {
                    progress.println(format!("{} {}", console::pad_str(console::style("Fetched").green().to_string().as_str(), PRINT_PADDING, Alignment::Right, None), water_right_no));
                    progress.inc(1);
                ...
                Err(err) => {
                    progress.println(format!("{} failed to fetch, {}", console::pad_str(console::style("Error").red().to_string().as_str(), PRINT_PADDING, Alignment::Right, None), err));

                    // use quadratic backoff for wait until retry
                    let wait = 2u64.pow(retry);
                    progress.println(format!("{}  will try again in {wait} seconds...", console::pad_str("", PRINT_PADDING, Alignment::Right, None)));
        ...
        progress.println(format!("{} exceeded amount of retries, will skip {water_right_no}", console::pad_str(console::style("Warning").yellow().to_string().as_str(), PRINT_PADDING, Alignment::Right, None)));
        progress.inc(1);
    }

    progress.finish_and_clear();
    match unfetched_reports.is_empty() {
        false => println!("{}, could not fetch: {}", console::style("Fetching done").magenta(), unfetched_reports.iter().map(|no| no.to_string()).collect::<Vec<String>>().join(", ")),
